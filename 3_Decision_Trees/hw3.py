...
def estimate_gini_impurity(feature_values, threshold, labels, polarity): 
    ...
    a = sum(numpy.logical_and(polarity(feature_values, threshold), labels == 1))
    b = sum(numpy.logical_and(polarity(feature_values, threshold), labels == -1))
    n = a+b
    if n == 0:
        gini_impurity = 1.0
    else:
        gini_impurity = 1-(a/n)**2-(b/n)**2
...
def estimate_gini_impurity_expectation(feature_values, threshold, labels):
    ...
    count_gt = sum(operator.gt(feature_values, threshold))
    count_le = sum(operator.le(feature_values, threshold))
    ...
    if n == 0 :
        ...
        expectation = (count_gt*gini_impurity_gt + count_le*gini_impurity_le)/n
...
def grid_search_split_midpoint(X, y): 
    ...
    m = X.shape[0]
    n = X.shape[1]
    grid = numpy.zeros((m-1, n))
    XT = numpy.transpose(X)
    for i in range(0, n):
        for j in range(0, m-1):
            grid[j][i] = estimate_gini_impurity_expectation(XT[i], thresholds[j][i], y)
    index = numpy.where(grid == numpy.amin(grid))
    best_feature = index[1][0]
    best_threshold = thresholds[index[0][0]][best_feature]
